...
The Python script `check.py` takes input data from an Excel spreadsheet to send HTTP requests to the official DPSG NaMi server and checks the "erweiterte Führungszeugnisse" for validity.
...
**Note**: A second script `check_pdf.py` has been added that can be used to check PDF files that have been generated by NaMi itself.
